...
def createRolesMessage():
    ...
    embed.set_image(
        url="https://cdn.discordapp.com/attachments/906689619205177415/909413127861465118/rolesImage.jpg"
    )
    return embed
        ...
        async for user in reaction.users():
            if message.author != user:
                role = discord.utils.get(
                    guild.roles, name=client.rolesIds[reaction.emoji]
                )
...
async def assign_role(client, payload):
    ...
    try:
        role = discord.utils.get(guild.roles, name=client.rolesIds[payload.emoji.name])
