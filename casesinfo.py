...
class CovidCases(Thread):
    ...
    def outputVic(self):
        melbCases = self.vic_soup.find_all('div', class_="ch-daily-update__statistics-item-text")[4].text
        time = getTimeZone('Australia/Melbourne')
        return melbCases + " cases acquired in Victoria (Last 24 hours) (Last updated: " + time + ")"
    
    def vicProgressBar(self):
        vaxProgressText = self.vic_soup.find_all('div', class_ = 'ch-daily-update__statistics-item-text')[1].text
        ...
        if full == True:
            ...
            progressResult = "VIC - Fully vaccinated: " + progressBar + " " + vaxProgressText + " / " + str(denom) + "%"
        return progressResult

    def outputNSW(self):
        time = getTimeZone('Australia/Melbourne')
        NSWCases = self.nsw_soup.find_all('span', class_="number")[0].text
        return NSWCases + " cases acquired in NSW (Last 24 hours) (Last updated: " + time + ")"
